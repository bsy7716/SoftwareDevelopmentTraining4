...
<body>
	<%
		...
		String ReviewType = "전체";
		String searchType = "최신순";
		String search = "";
	
		if(request.getParameter("ReviewType") != null) {
			ReviewType = request.getParameter("ReviewType");
		}
    ...
    <div class="container">
      ...
      <h1 class="my-4">리뷰&추천
       ...
       <div class="row">
		...
		<div class="row">
				<div class="col-md-10">
				<h4>영화 추천 및 리뷰 게시판입니다. 자유롭게 작성하세요!</h4>
	         	</div>
		        <form method="get" action="ReviewList.jsp" class="form-inline mt-3">
			        <select name="ReviewType" class="form-control mx-1 mt-2">
			          <option value="전체">전체</option>
			          <option value="리뷰" <%if(ReviewType.equals("리뷰")) out.println("selected");%>>리뷰</option>
			          <option value="추천" <%if(ReviewType.equals("추천")) out.println("selected");%>>추천</option>
			          <option value="기타" <%if(ReviewType.equals("기타")) out.println("selected");%>>기타</option>
			        </select>
			        ...
			        <button class="btn btn-outline-primary mx-1 mt-2">검색</button>
			      </form>
			        <%
					ArrayList<Review> ReviewList = new ArrayList<Review>();
			 	    ReviewList = new ReviewDAO().getList(ReviewType, searchType, search);
				 	%>
	         	<br><br>
					<table class="table table-striped" style="text-align:center;border:1px solid #dddddd">
						<thead>
							<tr>
								...
								<th style="backgroud-color:#eeeeee; text-align:center;" >타입</th>
								...
								<th style="backgroud-color:#eeeeee; text-align:center;" >평점</th>
						...
						<tbody>
							...
							<tr>
								...
								<td><a href="reviewcontentview?ReviewNo=${review.reviewNo}" class="btn btn-info btn-sm">보기</a></td>
								<td>${review.movieScore}</td>
								<td>${review.reviewDate}</td>
								<td><a href="reviewupdate?ReviewNo=${review.reviewNo}" class="btn btn-success btn-sm">수정</a></td>
								<td><a href="reviewdelete?ReviewNo=${review.reviewNo}" class="btn btn-danger btn-sm">삭제</a></td>
							</tr>
							</c:forEach>
						</tbody>
					</table>
